...
async function getUserObject() {
  ...
  if (response.ok) {
    ...
    appendError(response.error || 'Failed to get user object')
    return null
  }
}
...
async function loadResources() {
  ...
  if (!userObject.privileges.admin) {
    appendError('Insufficient privileges to access this resource.')
    return
  }
  ...
  const { Chart } = await import('../../js/modules/node_modules/chart.js/auto/auto.js')
  window.Chart = Chart
...
async function removeLoadingMask() {
  ...
  fadeAndRemove(document.getElementById("loading-mask"))
}
