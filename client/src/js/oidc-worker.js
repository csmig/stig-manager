...
async function getAuthorizationUrl(_redirectUri = redirectUri) {
  if (authorization[_redirectUri]) return authorization[_redirectUri]
  ...
  authorization[_redirectUri] = { redirect, codeVerifier: pkce.codeVerifier }
  return authorization[_redirectUri]
}
...
async function broadcastNoToken() {
  ...
  const redirect = await getAuthorizationUrl(`${baseRedirectUri}reauth.html`)
  bc.postMessage({ type: 'noToken', ...redirect })
}
...
async function exchangeCodeForToken({ code, codeVerifier, clientId: reqClientId = 'stig-manager', redirectUri: reqRedirectUri }) {
  if (authorization[reqRedirectUri] && authorization[reqRedirectUri].codeVerifier !== codeVerifier) {
    // verifier does not match the saved redirectUri
    console.error(logPrefix, 'Code verifier does not match the saved redirectUri', reqRedirectUri, authorization[reqRedirectUri])
  ...
  delete authorization[reqRedirectUri]
  const params = new URLSearchParams()
  params.append('grant_type', 'authorization_code')
  params.append('client_id', reqClientId)
